...
import { AiScoreBadge, AiScoreBreakdown } from './AiScoreBadge';
import type { Candidate, Vacancy, VacancyCandidate } from '@/api/types';

interface Props {
  vacancy: Vacancy;
  candidate: Candidate;
  /** Связка vacancy↔candidate — несёт AI-скор (aiScore/aiRecommendation/...). */
...
export function MatchCompensationRow({
  ...
  // Разбивку AI-скоринга раскрываем инлайн под строкой (не Popover — надёжнее
  // внутри Sheet, см. plan-ai-scoring §6.2).
  const [scoreOpen, setScoreOpen] = useState(false);

  return (
    <div className="group rounded-md border bg-muted/30">
      ...
      <AiScoreBreakdown vacancyId={vacancy.id} candidateId={candidate.id} open={scoreOpen} />
    </div>
  );
}
