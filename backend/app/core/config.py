...
class Settings(BaseSettings):
    ...
    #
    # ОБЯЗАНО быть больше celery `task_time_limit`
    # (screening_task_time_limit_min, см. celery_app.py):
    ...
    # а затем пометил бы нормально работающую сессию как error. Слишком
    # маленькое значение подтягивается вверх в close_stale_sessions().
    screening_processing_timeout_min: int = 45
    # Лимиты celery-задач скрининга (мин). Офлайн-STT записи считается на CPU
    # примерно в реальном времени, и дорожек у стерео-записи две, поэтому
    # часовая встреча законно занимает больше часа — прежние 30/35 минут
    # рубили её на середине, оставляя пустой транскрипт. soft бросает
    # SoftTimeLimitExceeded (задача успевает упасть штатно и уйти в retry),
    # hard — страховка от зависшего сокета к STT/S3.
    screening_task_soft_time_limit_min: int = 150
    screening_task_time_limit_min: int = 165
