...
export const candidatesApi = {
   ...
   * нужно YandexGPT. Большие резюме (10+ страниц HH) бэкенд парсит по
   * частям несколькими параллельными вызовами модели (таймаут одного вызова
   * на бэке — 60с), поэтому здесь запас 120с. Без override ky обрывает
   * запрос раньше → фронт ловит TimeoutError и показывает общий
   * «Не удалось распознать файл…» вместо реальной ошибки.
   */
  parseResumeText: (text: string) =>
    api
      .post('candidates/parse-resume-text', { json: { text }, timeout: 120_000 })
